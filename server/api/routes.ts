...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get("/api/verification-runs/:id/report.pdf", async (request, reply) => { const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const run = verificationRepository.get(id), result = verificationRepository.result(id); if (!run || !result) return reply.code(404).send({ error: "Completed verification report not found" }); const version = strategyRepository.version(run.strategyVersionId), trades = verificationRepository.trades(id, 1_000_000, 0) as Array<Record<string, unknown>>, audit = auditVerificationRun({ run, result, trades, costs: version?.configuration.costs, currentEngineVersion: SIMULATION_ENGINE_VERSION, currentMetricsVersion: METRICS_ENGINE_VERSION, currentMonteCarloVersion: MONTE_CARLO_ENGINE_VERSION, currentExportVersion: VERIFICATION_EXPORT_VERSION, currentWalkForwardVersion: WALK_FORWARD_ENGINE_VERSION, currentRobustnessVersion: ROBUSTNESS_ENGINE_VERSION, currentStressVersion: STRESS_ENGINE_VERSION, currentReportVersion: REPORT_ENGINE_VERSION }); const pdf = await verificationReportPdf({ run, version, result, audit, trades, logs: verificationRepository.logs(id) as Array<Record<string, unknown>> }); return reply.header("Content-Disposition", `attachment; filename=treidin-verification-${id}.pdf`).type("application/pdf").send(pdf); });
  ...
  app.get("/api/verification-runs/:id/trades.csv", async (request, reply) => { const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const run = verificationRepository.get(id); if (!run) return reply.code(404).send({ error: "Verification run not found" }); const version = strategyRepository.version(run.strategyVersionId); const metadata = { configurationHash: run.configurationHash, indicatorRegistryVersion: version?.indicatorRegistryVersion, engineVersion: run.engineVersion, metricsVersion: run.metricsVersion, monteCarloEngineVersion: run.monteCarloEngineVersion, walkForwardEngineVersion: run.walkForwardEngineVersion, robustnessEngineVersion: run.robustnessEngineVersion, stressEngineVersion: run.stressEngineVersion, exportVersion: run.exportVersion, reportEngineVersion: run.reportEngineVersion, randomSeed: run.randomSeed }; reply.hijack(); reply.raw.writeHead(200, { "Content-Type": "text/csv; charset=utf-8", "Content-Disposition": `attachment; filename=verification-${id}-trades.csv`, "Cache-Control": "no-store" }); const write = async (chunk: string) => { if (!reply.raw.write(chunk)) await once(reply.raw, "drain"); }; await write(tradeCsvHeader()); for (const row of verificationRepository.iterateTrades(id)) await write(tradeCsvLine(run.id, run.strategyVersionId, row, metadata)); reply.raw.end(); });
