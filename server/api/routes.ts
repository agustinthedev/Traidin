...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get("/api/verification-runs/:id/trades.csv", async (request, reply) => { const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const rows = verificationRepository.trades(id, 1000, 0) as Array<Record<string, unknown>>; const fields = ["sequence", "side", "entry_time", "exit_time", "entry_price", "exit_price", "quantity", "gross_pnl", "net_pnl", "fees", "return_pct", "r_multiple", "mae_pct", "mfe_pct", "holding_ms", "entry_reason", "exit_reason"]; const escape = (value: unknown) => `"${String(value ?? "").replaceAll("\"", "\"\"")}"`; const csv = [fields.join(","), ...rows.map((row) => fields.map((field) => escape(row[field])).join(","))].join("\n"); return reply.header("Content-Disposition", `attachment; filename=verification-${id}-trades.csv`).type("text/csv; charset=utf-8").send(csv); });
