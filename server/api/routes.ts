...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get("/api/verification-runs/:id/audit", async (request, reply) => { const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const run = verificationRepository.get(id); if (!run) return reply.code(404).send({ error: "Verification run not found" }); return auditVerificationRun({ run, result: verificationRepository.result(id), trades: verificationRepository.trades(id, 1_000_000, 0) as Array<Record<string, unknown>>, currentEngineVersion: SIMULATION_ENGINE_VERSION, currentMetricsVersion: METRICS_ENGINE_VERSION, currentMonteCarloVersion: MONTE_CARLO_ENGINE_VERSION, currentExportVersion: VERIFICATION_EXPORT_VERSION }); });
