...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get("/api/research-runs/:id/candidates", async (request, reply) => { const { id } = z.object({ id: z.string().uuid() }).parse(request.params); if (!researchRepository.get(id)) return reply.code(404).send({ error: "Research Run not found" }); const query = z.object({ limit: z.coerce.number().int().min(1).max(500).default(100), offset: z.coerce.number().int().min(0).default(0) }).parse(request.query); return { rows: researchRepository.candidates(id, query.limit, query.offset), total: researchRepository.candidateCount(id), limit: query.limit, offset: query.offset }; });
