...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get("/api/verification-runs/:id/report.pdf", async (request, reply) => { const { id } = z.object({ id: z.string().uuid() }).parse(request.params); const run = verificationRepository.get(id), result = verificationRepository.result(id); if (!run || !result) return reply.code(404).send({ error: "Completed verification report not found" }); const version = strategyRepository.version(run.strategyVersionId), trades = verificationRepository.trades(id, 1_000_000, 0) as Array<Record<string, unknown>>, audit = auditVerificationRun({ run, result, trades, currentEngineVersion: SIMULATION_ENGINE_VERSION, currentMetricsVersion: METRICS_ENGINE_VERSION, currentMonteCarloVersion: MONTE_CARLO_ENGINE_VERSION, currentExportVersion: VERIFICATION_EXPORT_VERSION }); const pdf = await verificationReportPdf({ run, version, result, audit, trades, logs: verificationRepository.logs(id) as Array<Record<string, unknown>> }); return reply.header("Content-Disposition", `attachment; filename=treidin-verification-${id}.pdf`).type("application/pdf").send(pdf); });
