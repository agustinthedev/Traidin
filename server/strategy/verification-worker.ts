...
function marketRegimes(trades: Array<{ entryTime: number; netPnl: string }>, engine: FeatureEngine, timeframe: string) { const groups: Record<string, { regime: string; trades: number; netProfit: number; wins: number }> = {}; for (const trade of trades) { const asOf = new Date(trade.entryTime), fast = engine.value({ indicator: "ema", parameters: { period: 50 }, timeframe }, asOf), slow = engine.value({ indicator: "ema", parameters: { period: 200 }, timeframe }, asOf); const regime = !Number.isFinite(fast) || !Number.isFinite(slow) ? "WARMUP_UNAVAILABLE" : Math.abs(fast / slow - 1) < .001 ? "TRANSITION" : fast > slow ? "BULL_TREND" : "BEAR_TREND"; const group = groups[regime] ??= { regime, trades: 0, netProfit: 0, wins: 0 }; group.trades++; group.netProfit += Number(trade.netPnl); group.wins += Number(trade.netPnl) > 0 ? 1 : 0; } return Object.values(groups).map((group) => ({ ...group, winRate: group.trades ? group.wins / group.trades * 100 : 0 })); }
