...
class VerificationWorker {
  ...
  private async execute(run: ReturnType<typeof verificationRepository.get> extends infer T ? Exclude<T, null> : never) {
    ...
    await verificationRepository.update(run.id, { stage: "METRICS", progress: .7, stageProgress: 0 }); const metrics = verificationMetrics(simulated.trades, simulated.equity); const initial = Number(run.options.initialBalance); const oosTime = run.requestedStart.getTime() + (run.requestedEnd.getTime() - run.requestedStart.getTime()) * Number(run.options.oosSplit), oosStart = new Date(oosTime); const oosSimulation = await simulate(config, engine, initial, undefined, () => verificationRepository.get(run.id)?.cancelRequested ?? false, rules, { start: oosStart, end: run.requestedEnd }); if (oosSimulation.cancelled) { await verificationRepository.update(run.id, { status: "CANCELLED", stage: "CANCELLED", completedAt: new Date() }); return; } const oosMetrics = verificationMetrics(oosSimulation.trades, oosSimulation.equity); const monteCarloCount = run.profile === "QUICK" ? 0 : Number(run.options.monteCarloCount); const mc = monteCarloCount ? monteCarlo(simulated.trades, initial, monteCarloCount, run.randomSeed, config) : null;
