...
export const RESEARCH_ENGINE_VERSION = "2026.08.random-restricted.1";
...
function metricSummary(simulated: Awaited<ReturnType<typeof simulate>>) { const metrics = verificationMetrics(simulated.trades, simulated.equity); return { trades: metrics.tradeCount, netProfit: metrics.netProfit, return: metrics.netProfitPct, profitFactor: metrics.profitFactor, expectancy: metrics.expectancy, maxDrawdownPct: metrics.maxDrawdownPct, fees: metrics.totalFees, equity: simulated.equity.filter((_, index) => index % Math.max(1, Math.ceil(simulated.equity.length / 200)) === 0) }; }
...
export class ResearchWorker {
  ...
  private async execute(run: ReturnType<typeof researchRepository.get> extends infer T ? NonNullable<T> : never) {
    ...
    const finalists: Array<{ id: string; config: StrategyConfig; score: number }> = [];
    ...
    for (const finalist of selected) { const holdout = metricSummary(await simulate(finalist.config, engine, input.initialBalance, undefined, () => researchRepository.get(run.id)?.status === "CANCELLING", {}, periods.holdout)); const candidate = researchRepository.candidate(finalist.id); if (!candidate) continue; const status = passes(holdout, input) ? "COMPLETED" : "HOLDOUT_REJECTED"; await researchRepository.updateCandidate(finalist.id, { status, metrics: { ...candidate.metrics, holdout }, score: finalist.score, rejectionStage: status === "COMPLETED" ? null : "HOLDOUT", rejectionReason: status === "COMPLETED" ? null : "QUALITY_FILTER" }); }
