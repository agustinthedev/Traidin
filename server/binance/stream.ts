...
export class BinanceKlineStream {
  ...
  start() { if (!this.stopped) return; this.stopped = false; this.watchdog = setInterval(() => { if (this.socket?.readyState === WebSocket.OPEN && this.lastMessageAt && Date.now() - this.lastMessageAt > 15_000) { void this.handlers.onStale(); this.socket.close(4000, "stale stream"); } }, 3000); this.connect(); }
