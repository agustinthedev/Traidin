...
`GET /api/verification-runs/:id/trades.csv` streams every persisted simulated trade in sequence order as RFC-style quoted CSV with entry/exit, quantity, P&L, costs, excursions, holding time and reason codes. The report exposes the same endpoint as **Export CSV**.
