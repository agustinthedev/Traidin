...
type MonteCarlo = { count: number; model?: string; pathsShown?: number; paths?: number[][]; percentilePaths?: { p05: number[]; median: number[]; p95: number[] }; initialBalance: number; p05FinalEquity: number; medianFinalEquity: number; p95FinalEquity: number; probabilityOfProfit: number; ruinedPathCount?: number; ruinProbabilityPct?: number; earliestRuinTradeIndex?: number | null; medianRuinTradeIndex?: number | null; minimumSimulatedEquity?: number; minimumSimulatedDrawdownPct?: number };
...
export default function MonteCarloPaths({ data }: { data: MonteCarlo }) {
  ...
  return <section className="panel monte-carlo-paths"><div className="panel-title"><span>MONTE CARLO PATHS</span><small>{data.pathsShown ?? data.paths.length} / {data.count} SIMULATIONS · P5 / MEDIAN / P95</small></div><canvas ref={host} className="monte-carlo-chart" aria-label="Monte Carlo equity paths" /><div className="monte-carlo-legend"><span><i className="mc-path" /> Simulated paths</span><span><i className="mc-p05" /> P5 {fmtNum(data.p05FinalEquity, 0)}</span><span><i className="mc-median" /> Median {fmtNum(data.medianFinalEquity, 0)}</span><span><i className="mc-p95" /> P95 {fmtNum(data.p95FinalEquity, 0)}</span><span><i className="mc-initial" /> Initial {fmtNum(data.initialBalance, 0)}</span><span>Profit probability {fmtNum(data.probabilityOfProfit, 1)}%</span><span>Ruin {fmtNum(data.ruinProbabilityPct, 1)}% ({data.ruinedPathCount ?? 0})</span><span>Earliest / median ruin trade {data.earliestRuinTradeIndex ?? "-"} / {data.medianRuinTradeIndex ?? "-"}</span><span>Minimum equity {fmtNum(data.minimumSimulatedEquity, 2)} · DD {fmtNum(data.minimumSimulatedDrawdownPct, 2)}%</span><span>Model {data.model ?? "-"}</span></div></section>;
}
